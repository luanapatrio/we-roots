<?php
// Configurações do banco de dados (mesmas do login.php)
$servername = "localhost";
$username = "root";
$password = "";
$dbname = "weroots_db";

$conn = new mysqli($servername, $username, $password, $dbname);

if ($conn->connect_error) {
    die("Falha na conexão: " . $conn->connect_error);
}
// echo "Clicou no botão";

// Alteração para incluir cadastro e login
// Processar formulário de cadastro
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["signup"])) {
    // ... (Receber, sanitizar e validar dados do formulário de cadastro) ...
    
    $nome = $_POST["username"];
    $email = $_POST["email"];
    $senha = $_POST["password"];
    $telefone = $_POST["telefone"];

    // Sanitizar entradas (IMPORTANTE para segurança!)
    $nome = $conn->real_escape_string($nome);
    $email = $conn->real_escape_string($email);
    // $senha = password_hash($senha, PASSWORD_DEFAULT); // Hash da senha com salting!
    $senha = md5($senha); // Hash da senha com salting!
    $telefone = $conn->real_escape_string($telefone);
    // $senha = password_hash($senha, PASSWORD_DEFAULT); // Hash da senha

    // Validação dos campos - Verifique se algum campo está vazio
    if (empty($nome) || empty($email) || empty($senha) || empty($telefone)) {
        $erro = "Por favor, preencha todos os campos.";
    } else {
        // Sanitização e validação adicional (se necessário)
        $nome = $conn->real_escape_string($nome);
        $email = $conn->real_escape_string($email);
        $senha = password_hash($senha, PASSWORD_DEFAULT); // Hash da senha
        $telefone = $conn->real_escape_string($telefone);


        // Verificar se o email já existe (adicionei essa validação para mais segurança)
        $sql_check = "SELECT * FROM usuarios WHERE email = '$email'";
        $result_check = $conn->query($sql_check);
        if ($result_check->num_rows > 0) {
            $erro = "Este email já está cadastrado.";
        } else {
            $sql = "INSERT INTO usuarios (nome, email, senha, telefone) VALUES ('$nome', '$email', '$senha', '$telefone')";

            if ($conn->query($sql) === TRUE) {
                $mensagem = "Cadastro realizado com sucesso!";
            } else {
                $erro = "Erro ao cadastrar: " . $conn->error;
            }
        }
      
    }
    // echo "Clicou no cadastro" . var_dump($erro) ;
}

// Processar formulário de login
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["signin"])) {
    // ... (Receber, sanitizar e validar dados do formulário de login) ...
    
    $email = $_POST["email"];
    $senha = $_POST["password"];
    
    // Sanitizar entradas (IMPORTANTE para segurança!)
    $email = $conn->real_escape_string($email);
    $senha = $conn->real_escape_string($senha); // Em produção, use password_hash() e password_verify()
    
    $sql = "SELECT * FROM usuarios WHERE email = '$email'";
    $result = $conn->query($sql);

    
    if ($result->num_rows > 0) {

        $row = $result->fetch_assoc();
        
        if (md5($senha) == $row["senha"]) {
            // echo "Clicou no login" . var_dump(md5($senha)) . var_dump($row["senha"]);
            session_start();
            $_SESSION["usuario_logado"] = $email;
            header("Location: ../home/home.html");
            exit();
        } else {
            $erro = "Senha incorreta.";
        }
    } else {
        $erro = "Usuário não encontrado.";
    }
}

// Fim da alteração
/*
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["signup"])) {
    $nome = $_POST["username"];
    $email = $_POST["email"];
    $senha = $_POST["password"];
    $telefone = $_POST["telefone"];

    // Sanitizar entradas (IMPORTANTE para segurança!)
    $nome = $conn->real_escape_string($nome);
    $email = $conn->real_escape_string($email);
    // $senha = password_hash($senha, PASSWORD_DEFAULT); // Hash da senha com salting!
    $senha = md5($senha); // Hash da senha com salting!
    $telefone = $conn->real_escape_string($telefone);


    $sql = "INSERT INTO usuarios (nome, email, senha, telefone) VALUES ('$nome', '$email', '$senha', '$telefone')";

    if ($conn->query($sql) === TRUE) {
        echo "<p>Cadastro realizado com sucesso!</p>";
    } else {
        echo "Erro: " . $sql . "<br>" . $conn->error;
    }
}
*/
$conn->close();
?>

<!DOCTYPE html>
<!-- ... (Seu código HTML do formulário de cadastro aqui) ... -->
<html lang="en" >
  <head>          
    <meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0">
    <meta charset="utf-8">
    <link rel="stylesheet" type="text/css" href="estilo.css"><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;700;800&display=swap" rel="stylesheet">
    <link rel="shortcut icon" href="img/logo.png" type="image/x-icon">
  </head>
  <body>
    <!-- LOGO -->
    <nav class="navbar navbar-expand-lg destaque py-4 px-2" id="navbar">
  </nav>
  <!-- CADASTRO -->
    <div class="main">
      <div class="container a-container" id="a-container">
        <form class="form" id="a-form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
          <h2 class="form_title title">Crie uma conta</h2>
          <div class="form__icons"><img class="form__icon" src="data:image/svg+xml;base64," alt=""><img class="form__icon" src="data:image/svg+xml;base64,"><img class="form__icon" src="data:image/svg+xml;base64,"></div><span class="form__span">ou use seu e-mail para se cadastrar</span>
          <input class="form__input" type="text" placeholder="Nome" name ="username">
          <input class="form__input" type="text" placeholder="Email" name="email">
          <input class="form__input" type="password" placeholder="Senha" name="password">
          <input class="form__input" type="telefone" placeholder="Telefone" name="telefone">

          <!-- Exibir mensagem de erro antes do botão -->
          <?php if (isset($erro)): ?>
              <div class="error-message">
                  <p style="color: red;"><?php echo htmlspecialchars($erro); ?></p> </div>
          <?php endif; ?>

          <!-- Exibir mensagem de sucesso antes do botão -->
          <?php if (isset($mensagem)): ?>
              <div class="error-message">
                  <p style="color: greenyellow;"><?php echo htmlspecialchars($mensagem); ?></p> </div>
          <?php endif; ?>

          <!-- <a href="../index.html"><button class="form__button button switch-btn" type='submit' name ="signup">CADASTRAR</button></a> -->
          <a><button class="form__button button switch-btn" type='submit' name ="signup">CADASTRAR</button></a>
        </form>
      </div>
      <div class="container b-container" id="b-container">
        <form class="form" id="a-form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            <!-- ... (campos do formulário) ... -->
            <h2 class="form_title title">Login</h2>
            <div class="form__icons"><img class="form__icon" src="data:image/svg+xml;base64," alt=""><img class="form__icon" src="data:image/svg+xml;base64,"><img class="form__icon" src="data:image/svg+xml;base64,"></div><span class="form__span">ou use sua conta do e-mail</span>
            <input class="form__input" type="text" placeholder="Email" name = "email">
            <input class="form__input" type="password" placeholder="Senha" name = "password">
            <a href="#" class="form__link"> Esqueceu a senha?</a>
            <!-- <a href="../loginadmin/login.html" class="form__link"> É um administrador? clique aqui!</a> -->
            <a href="../loginadmin/login.php" class="form__link"> É um administrador? clique aqui!</a>
            <button class="form__button button switch-btn" type='submit' name ="signin">LOGIN</button>
        </form>
        
        <!-- Formulário antigo | HTML -->
        <!-- <form class="form" id="b-form" method="post"> -->
          <!-- <h2 class="form_title title">Login</h2> -->
          <!-- <div class="form__icons"><img class="form__icon" src="data:image/svg+xml;base64," alt=""><img class="form__icon" src="data:image/svg+xml;base64,"><img class="form__icon" src="data:image/svg+xml;base64,"></div><span class="form__span">ou use sua conta do e-mail</span> -->
          <!-- <input class="form__input" type="text" placeholder="Email" name = "email"> -->
          <!-- <input class="form__input" type="password" placeholder="Senha" name = "password"> -->
          <!-- <a href="#" class="form__link"> Esqueceu a senha?</a> -->
          <!-- <a href="../loginadmin/login.html" class="form__link"> É um administrador? clique aqui!</a> -->
          <!-- <a href="../loginadmin/login.php" class="form__link"> É um administrador? clique aqui!</a> -->
          <!-- <button class="form__button button switch-btn" name = "signin">LOGIN</button> -->
        <!-- </form> -->
      </div>
      <div class="switch" id="switch-cnt">
        <div class="switch__circle"></div>
        <div class="switch__circle switch__circle--t"></div>
        <div class="switch__container" id="switch-c1">
          <h2 class="switch__title title">Bem vindo!</h2>
          <p class="switch__description description">Seja atualizado de todas as informações da fazenda urbana! </p>
          <button class="switch__button button switch-btn">ENTRAR</button>
        </div>
        <div class="switch__container is-hidden" id="switch-c2">
          <h2 class="switch__title title">Olá WeRooter!</h2>
          <p class="switch__description description">Para ser um membro da WeRoots, cadastre-se!</p>
          <button class="switch__button button switch-btn">CADASTRE-SE</button>
        </div>
      </div>
    </div>
  </body>
  
</html>
  <script  src="SIGN UP & SIGN IN PAGE.js"></script>
</body>
<!-- Até aqui -->




<!-- ... (resto do seu HTML, incluindo o formulário de login) ... -->